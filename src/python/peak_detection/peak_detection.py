...
peaks=open('peaks%s-%s-%s.csv'%(region,timeframe,spatial_division.replace(".","").replace("/","")),'w')
#format: area,hour,date->percentage
for l in  chiamate_orarie.collect():
    print >>peaks, "%s,%s,%s,%s"%(l[0][0],l[0][3],l[0][2],l[1]/np.mean(list(presenze_medie[(l[0][0],l[0][1],l[0][2])])))